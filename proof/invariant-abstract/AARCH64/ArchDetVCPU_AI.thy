...
lemma cur_vcpu_in_cur_domain_lift_strong:
  assumes [wp]: "\<And>P. f \<lbrace>\<lambda>s. P (arm_current_vcpu (arch_state s))\<rbrace>"
                ...
                "\<And>P t . f \<lbrace>etcb_at (\<lambda>t. P (tcb_domain t)) t\<rbrace>"
...
lemma cur_vcpu_in_cur_domain_lift:
  assumes "\<And>P. f \<lbrace>\<lambda>s. P (arm_current_vcpu (arch_state s))\<rbrace>"
          "\<And>P. f \<lbrace>\<lambda>s. P (vcpu_tcbs_of s)\<rbrace>"
          "\<And>P. f \<lbrace>\<lambda>s. P (cur_domain s)\<rbrace>"
          "\<And>P. f \<lbrace>\<lambda>s. P (ekheap s)\<rbrace>"
  shows "f \<lbrace>cur_vcpu_in_cur_domain\<rbrace>"
...
crunch set_vm_root, set_global_user_vspace
  for cur_vcpu_in_cur_domain[wp]: cur_vcpu_in_cur_domain
  (wp: cur_vcpu_in_cur_domain_lift)
...
crunch next_domain, vcpu_flush
  for hyp_refs_of[wp]: "\<lambda>s. P (state_hyp_refs_of s)"
  and valid_objs[wp]: valid_objs
  (simp: crunch_simps wp: crunch_wps ignore: vcpu_update)
...
lemma ethread_set_no_change_etcb_at:
  assumes x: "\<And>tcb. P (f tcb) = P tcb"
  shows      "\<lbrace>etcb_at P t\<rbrace> ethread_set f t' \<lbrace>\<lambda>_. etcb_at P t\<rbrace>"
  apply (simp add: ethread_set_def set_eobject_def etcb_at_def)
  apply wpsimp
  apply (clarsimp simp: x get_etcb_def split: option.splits)
  done

crunch handle_interrupt
  for etcb_at_domain[wp]: "etcb_at (\<lambda>t. P (tcb_domain t)) t"
  and cur_domain[wp]: "\<lambda>s. P (cur_domain s)"
  (wp: ethread_set_no_change_etcb_at)
...
crunch set_extra_badge, handle_fault
  for vcpus_of[wp]: "\<lambda>s. P (vcpus_of s)"
  (wp: dxo_wp_weak crunch_wps transfer_caps_loop_pres
   simp: crunch_simps
   ignore: set_simple_ko possible_switch_to)
...
crunch handle_interrupt
  ...
  (wp: cur_vcpu_in_cur_domain_lift_strong dxo_wp_weak crunch_wps
   simp: crunch_simps
   ignore: set_simple_ko tcb_sched_action empty_slot_ext)


\<comment> \<open>handle_event\<close>

lemma retype_region_ext_valid_cur_vcpu_kheap_update[wp]:
  "retype_region_ext ptrs type \<lbrace>\<lambda>s. cur_vcpu_in_cur_domain (kheap_update kh s)\<rbrace>"
  unfolding retype_region_ext_def
  apply (wpsimp simp_del: fun_upd_apply simp: foldr_fun_upd_value)
  by (auto simp: cur_vcpu_in_cur_domain_defs etcb_at'_def default_ext_def opt_map_def
...
lemma delete_objects_valid_cur_vcpu[wp]:
  ...
  by (auto simp: cur_vcpu_in_cur_domain_defs etcb_at'_def detype_def detype_ext_def
                 wrap_ext_det_ext_ext_def opt_map_def
          split: option.splits)
...
\<comment> \<open>FIXME: move\<close>
lemma cur_vcpu_in_cur_domain_current_vcpu_None:
...
lemma vcpu_invalidate_active_valid_cur_vcpu[wp]:
  ...
  by (wpsimp simp: cur_vcpu_in_cur_domain_current_vcpu_None)
...
crunch
  cap_insert, cap_move, set_thread_state, unbind_maybe_notification, unbind_notification, cancel_all_ipc,
  suspend, cancel_all_signals, delete_asid_pool, unmap_page, delete_asid, unmap_page_table,
  cancel_badged_sends
  for vcpu_tcbs_of[wp]: "\<lambda>s. P (vcpu_tcbs_of s)"
  and arm_current_vcpu[wp]: "\<lambda>s. P (arm_current_vcpu (arch_state s))"
  and cur_vcpu_in_cur_domain[wp]: cur_vcpu_in_cur_domain
  (wp: cur_vcpu_in_cur_domain_lift_strong crunch_wps dxo_wp_weak
   ...
   ignore: cap_move_ext set_simple_ko tcb_sched_action)
...
crunch cancel_ipc, thread_set, set_mcpriority, set_priority, bind_notification
  for vcpu_tcbs_of[wp]: "\<lambda>s. P (vcpu_tcbs_of s)"
  and arm_current_vcpu[wp]: "\<lambda>s. P (arm_current_vcpu (arch_state s))"
  and cur_vcpu_in_cur_domain[wp]: cur_vcpu_in_cur_domain
...
crunch
  ...
  perform_asid_control_invocation, perform_asid_pool_invocation
  for cur_vcpu_in_cur_domain[wp]: cur_vcpu_in_cur_domain
...
lemma set_thread_state_runnable_scheduler_action:
  ...
  apply (simp add: set_thread_state_ext_def set_object_def get_object_def set_scheduler_action_def)
