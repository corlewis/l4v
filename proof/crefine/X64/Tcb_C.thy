...
lemma invokeTCB_SetTLSBase_ccorres:
  ...
  apply (cinit lift: thread_' tls_base_')
   ...
   apply vcg
  apply (fastforce simp: tlsBaseRegister_def X64.tlsBaseRegister_def
                         invs_weak_sch_act_wf TLS_BASE_def FS_BASE_def
                  split: if_split)
  done
...
lemma invokeTCB_SetFlags_ccorres:
  ...
  apply (cinit' lift: thread_' clear_' set_' call_' simp: invokeSetFlags_def setFlags_def postSetFlags_def)
   ...
   apply (rule ccorres_symb_exec_r)
     ...
     apply (rule ccorres_split_nothrow)
        ...
        apply ceqv
       apply (rule ccorres_split_nothrow_dc)
          apply (rule_tac R=\<top> and R'="{s. flags_' s = flags && ~~ clears || sets && mask numFlags}"
  ...
  by (auto dest: invs_valid_objs' elim!: valid_objsE'
           simp: X64.badgeRegister_def badgeRegister_def Kernel_C.badgeRegister_def X64.capRegister_def
                 fromPAddr_def ThreadState_defs Kernel_C.RDI_def Kernel_C.RSI_def projectKOs
