...
lemma mcpriority_tcb_at'_le_ucast:
  ...
  by (frule mcpriority_tcb_at'_prio_bounded) (auto simp: ucast_le_8_32_equiv)
...
lemma bindNTFN_alignment_junk:
  ...
  apply (rule is_aligned_neg_mask_weaken)
  apply (erule aligned_add_aligned)
...
lemma invokeTCB_SetTLSBase_ccorres:
  ...
  apply (cinit lift: thread_' tls_base_')
   ...
   apply vcg
  apply (fastforce simp: tlsBaseRegister_def ARM_HYP.tlsBaseRegister_def
...
lemma invokeTCB_SetFlags_ccorres:
  ...
  by (auto dest: invs_valid_objs' elim!: valid_objsE'
           simp: ARM_HYP.badgeRegister_def badgeRegister_def Kernel_C.badgeRegister_def
                 fromPAddr_def ThreadState_defs Kernel_C.R0_def Kernel_C.R1_def projectKOs
