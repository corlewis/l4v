...
lemma frame_gp_registers_convs:
  ...
  "length RISCV64_H.gpRegisters = unat n_gpRegisters"
  "n < length RISCV64_H.frameRegisters \<Longrightarrow>
     index frameRegistersC n = register_from_H (RISCV64_H.frameRegisters ! n)"
  "n < length RISCV64_H.gpRegisters \<Longrightarrow>
     index gpRegistersC n = register_from_H (RISCV64_H.gpRegisters ! n)"
  apply (simp_all add: RISCV64_H.gpRegisters_def RISCV64_H.frameRegisters_def
                       RISCV64.gpRegisters_def n_gpRegisters_def
                       RISCV64.frameRegisters_def n_frameRegisters_def
          ...
          apply (rule ccorres_rhs_assoc2, rule ccorres_split_nothrow_novcg)
            ...
            apply (rule ccorres_rhs_assoc2, rule ccorres_split_nothrow_novcg)
                ...
                apply (rule_tac F="\<lambda>n. sysargs_rel args buffer
                                     and sysargs_rel_n args buffer (n + length RISCV64_H.frameRegisters + 2)
                 ...
                 apply arith
                apply (simp add: RISCV64_H.gpRegisters_def size_gpRegisters word_bits_def
                                 RISCV64_H.frameRegisters_def size_frameRegisters)
               apply simp
               apply (rule ceqv_refl)
              apply (ctac(no_vcg) add: getRestartPC_ccorres)
               apply simp
...
lemma msg_registers_convs:
  "length RISCV64_H.msgRegisters = unat n_msgRegisters"
  "n < length RISCV64_H.msgRegisters \<Longrightarrow>
     index msgRegistersC n = register_from_H (RISCV64_H.msgRegisters ! n)"
  apply (simp_all add: msgRegisters_unfold
                       RISCV64.msgRegisters_def n_msgRegisters_def
          ...
          apply (rule ccorres_symb_exec_l)
             apply (rule ccorres_symb_exec_l[OF _ _ _ empty_fail_getThreadState])
               ...
               apply (rule ccorres_if_lhs)
                ...
                apply (simp add: Collect_const_mem RISCV64_H.badgeRegister_def
...
lemma checkValidIPCBuffer_ccorres:
  ...
  apply (simp add:checkValidIPCBuffer_def RISCV64_H.checkValidIPCBuffer_def)
  apply (cases "isArchFrameCap cp \<and> \<not> isDeviceCap cp")
   ...
   apply (cinit lift: vptr_' cap_')
    apply (simp add: RISCV64_H.checkValidIPCBuffer_def if_1_0_0 ipcBufferSizeBits_def
                del: Collect_const)
    apply csymbr
  ...
  apply (cases "isArchFrameCap cp")
   ...
   apply clarsimp
   apply (cinit lift: vptr_' cap_')
    apply (simp add: RISCV64_H.checkValidIPCBuffer_def del: Collect_const)
    apply csymbr
    apply (rule ccorres_cond_false_seq)
    apply simp
    apply csymbr
  ...
  apply (cinit' lift: vptr_' cap_')
   apply csymbr
   apply (simp add: RISCV64_H.checkValidIPCBuffer_def cap_get_tag_isCap)
...
where
 "isValidVTableRoot_C cap \<equiv> cap_get_tag cap = scast cap_page_table_cap
             \<and> to_bool (capPTIsMapped_CL (cap_page_table_cap_lift cap))"
...
lemma isValidVTableRoot_conv:
  ...
  apply (clarsimp simp: isValidVTableRoot_C_def
                        ...
                        RISCV64_H.isValidVTableRoot_def)
  ...
  apply (case_tac "cap_get_tag cap' = scast cap_page_table_cap")
   apply (clarsimp simp: cap_page_table_cap_def isArchCap_tag_def2)
...
lemma invokeTCB_SetTLSBase_ccorres:
  ...
  apply (cinit lift: thread_' tls_base_')
   ...
   apply vcg
  apply (fastforce simp: tlsBaseRegister_def RISCV64.tlsBaseRegister_def
...
lemma scast_seL4_TCBFlag_MASK_numFlags[simp]:
  "scast seL4_TCBFlag_MASK = mask numFlags"
  by (clarsimp simp: seL4_TCBFlag_MASK_def numFlags_def)
...
lemma invokeTCB_SetFlags_ccorres:
  ...
  by (auto dest: invs_valid_objs' elim!: valid_objsE'
           simp: RISCV64.badgeRegister_def badgeRegister_def Kernel_C.badgeRegister_def
                 fromPAddr_def ThreadState_defs Kernel_C.a0_def Kernel_C.a1_def
                 pred_tcb_at'_def obj_at'_def ct_in_state'_def typ_heap_simps' mask_2pm1
                 valid_obj'_def valid_tcb'_def word_or_le_mask[simplified mask_2pm1] word_and_le
                 word_bool_le_funs)
